...
const Navbar = () => {
  ...
  return (
    <nav className={`p-1 font-serif flex justify-between items-center shadow-md transition duration-300 ${bgColor}`}>
      {/* Logo Section */}
      ...
      <ul className="flex items-end justify-center text-blue-600 space-x-6 list-none h-full mt-10">
  {['Home', 'Solutions', 'Services', 'Technology', 'About'].map(item => (
   <li
   key={item}
   className={`relative transition duration-200 ease-in-out h-full flex items-center`}
   onMouseEnter={() => {
